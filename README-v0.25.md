...
- Ammo dice are capped at `min(RoF, rounds remaining - 1)`.
- An empty tracked weapon cannot attack; one remaining round permits an attack with zero ammo dice.
...
- Ammunition expenditure is one round plus the sum of the ammo D6s, capped at the rounds loaded.
...
7. Confirm a weapon at zero rounds cannot attack and a weapon at one round allows zero ammo dice.
